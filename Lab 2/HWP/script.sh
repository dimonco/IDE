...
cat report.txt | msmtp --host=smtp.mail.ru --port=465 --protocol=smtp --auth=on --user=dimon_co@mail.ru --tsl==on --tsl-trust-file=/etc/ssl/certs/ca-certificates.crt -f dimon_co@mail.ru -t dimon_co@mail.ru
rm report.txt
